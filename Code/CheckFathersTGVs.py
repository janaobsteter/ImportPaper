...
for rep in rep:
    for scenario in scenarios:
        ...
        allBulls.loc[:, "scenario"] = scenario
        allBulls.loc[:, "rep"] = rep
        ...
        homeFathersMean.loc[:, "scenario"] = scenario
        homeFathersMean.loc[:, "rep"] = rep
        # Concatenate the dataframes
...
# Write the files
allBulls_final.groupby(['scenario', 'Group', 'Generation'])[['gvNormUnres1', 'gvNormUnres3']].mean().to_csv("AllBulls_TGVs.csv")
# Write the file for the bulls used in home population
homeFathersMean_final.groupby(['scenario', 'Group', 'Generation'])[['gvNormUnres1', 'gvNormUnres3']].mean().to_csv("HomeFathers_TGVs.csv")